...
def process_cvat_xml(xml_file, output_dir):
    ...
    for frameid in sorted(frames.keys()):
        ...
        for objid in objids:
            obj = frame[objid]
            if 'hito_right' in obj.values() | 'hito_left' in obj.values():
                hito_present = 1
   ...
   # Generate Data set
    video_absPath = os.path.join(output_dir, video_name)
...
def generate_dataset(video_absPath, anno_dir, frames):
    ...
    """
    [video_dir, video_name]  =os.path.split(video_absPath)
...
def main():
    args = parse_args()
    frames = process_cvat_xml(args.cvat_xml, args.output_dir)
